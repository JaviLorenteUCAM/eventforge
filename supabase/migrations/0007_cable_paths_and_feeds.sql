-- ===========================================================================
-- EventForge · 0007 · Cables trazados a mano y puntos de acometida
-- ===========================================================================
--
-- Qué cambia y por qué
-- --------------------
-- 1. Un cable deja de ser forzosamente una línea recta entre dos aparatos.
--    En un montaje real la manguera bordea paredes, rodea el escenario y da
--    vueltas; esos metros cuentan, y el listado de material los necesita.
--    Se guardan los puntos intermedios del trazo en `waypoints`.
--
--    Formato: [{"x": 1.25, "y": 3.4}, ...] en METROS, mismo sistema de
--    coordenadas que `plan_objects` (x izquierda→derecha, y arriba→abajo).
--    Lista vacía = cable recto, que es como se comportaban todos hasta ahora.
--
-- 2. Aparece el tipo de objeto `network_source`: el «punto de red principal»
--    de la sala, la roseta que trae Internet. Hasta ahora solo los switches y
--    los routers contaban como origen de red, así que no había forma de
--    representar la acometida del edificio. Su equivalente eléctrico,
--    `power_source` (el «punto de luz»), ya existía.
--
...
do $do$
begin
  if not exists (select 1 from pg_constraint where conname = 'plan_connections_waypoints_array') then
    alter table public.plan_connections
      add constraint plan_connections_waypoints_array
      check (jsonb_typeof(waypoints) = 'array');
  end if;
  if not exists (select 1 from pg_constraint where conname = 'scenario_connections_waypoints_array') then
    alter table public.scenario_connections
      add constraint scenario_connections_waypoints_array
      check (jsonb_typeof(waypoints) = 'array');
  end if;
end$do$;

comment on column public.plan_connections.waypoints is
  'Puntos intermedios del cable, en metros: [{"x":1.2,"y":3.4}]. Vacío = recto.';

-- --- 2. Punto de red principal --------------------------------------------
-- `plan_objects.kind` es texto libre, pero la biblioteca sí tenía lista
-- cerrada. Se amplía para poder dar de alta ahí el nuevo tipo.
...
-- --- 3. Las copias arrastran también el trazado ----------------------------
-- Las tres funciones son las de 0005 con la nueva columna añadida a los
-- INSERT/SELECT de cables. Todo lo demás queda igual.
